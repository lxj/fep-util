...
fepUtil.ejsCompile = function(req, res, next) {
  ...
  if (extname == "" && fs.existsSync(__dirname + reqUrl.replace(/\/+$/, '/') + 'index.html')) {
    reqUrl = reqUrl.replace(/\/+$/g, '/') + 'index.html';
    extname = path.extname(reqUrl);
    realextname = extname.replace(/(?:\?|#)+.*/ig, '');
  }
  if (realextname === '.html' || realextname === '.htm' || realextname === '.shtml' || realextname === '.ejs') {
    var fileStr = fs.readFileSync(__dirname + reqUrl);
...
fepUtil.stylusCompile = function(req, res, next) {
  ...
  if (realextname === '.styl') {
    var fileContent = grunt.file.read(__dirname + reqUrl);

    stylus(fileContent.toString()).set('filename', __dirname + reqUrl).render(function(err, css) {
      if (err) throw err;
      console.log(css);
      res.end(css);
    });
